...
{
    ("Calibration clamps both extremes and is monotonic", () =>
    {
        var calibration = new Calibration();
        Near(0, calibration.Normalize(0)); Near(1, calibration.Normalize(255));
        for (var n = 1; n <= 255; n++) Require(calibration.Normalize(n) >= calibration.Normalize(n - 1));
    }),
    ("Invalid and nonfinite calibration cannot reach hardware", () =>
    {
        Throws(() => new Calibration(100, 102).Validate());
        Throws(() => new Calibration(double.NaN, 180).Validate());
        Throws(() => new Calibration().Normalize(double.PositiveInfinity));
        Throws(() => new DisplayProfile(true, 80, 20).Validate());
    }),
    ("BGRA red and blue receive the correct weights", () =>
    {
        Near(77, LightMeter.MeasureBgra([0, 0, 255, 255]));
        Near(29, LightMeter.MeasureBgra([255, 0, 0, 255]));
        Near(255, LightMeter.MeasureBgra([255, 255, 255, 255]));
    }),
    ("Small bright/dark objects do not dominate the camera reading", () =>
    {
        var image = new byte[100 * 4];
        for (var i = 0; i < 100; i++)
        {
            var level = i < 10 ? (byte)0 : i >= 90 ? (byte)255 : (byte)100;
            image[i * 4] = image[i * 4 + 1] = image[i * 4 + 2] = level; image[i * 4 + 3] = 255;
        }
        Near(100, LightMeter.MeasureBgra(image));
        Throws(() => LightMeter.MeasureBgra([]));
    }),
    ...
    ("Bright or dark objects across two fifths of the frame cannot dominate the room", () =>
    {
        foreach (var patch in new byte[] { 0, 255 })
        {
            ...
            Require(Math.Abs(LightMeter.MeasureBgra(frame) - 80) > 20);
        }
    }),
    ("Spatial meter follows room-wide changes through foreground clutter within three seconds", () =>
    {
        ...
        var coordinator = new BrightnessCoordinator([new(screen, profile)], new(), true, true);
        foreach (var light in new byte[] { 80, 0, 80 })
        {
            ...
            Near(profile.Target(light, 0), screen.Level, 1);
        }
    }),
    ...
    ("Near-black sensor noise stays finite and cannot cause a large brightness swing", () =>
    {
        ...
        var coordinator = new BrightnessCoordinator([new(screen, new(true, 10, 100, 30, 1))], new(), true, true);
    ...
    ("One light source drives both displays with independent ranges", () =>
    {
        ...
        var coordinator = new BrightnessCoordinator([
            new(laptop, new DisplayProfile(true, 20, 80)), new(external, new DisplayProfile(true, 10, 60))], new Calibration(0, 200, 1));
    ...
    ("Manual change on either display pauses before any group writes", () =>
    {
        ...
        Require(result.ManualOverride); Require(a.Writes == 0 && b.Writes == 0);
    }),
    ...
    ("A recovered monitor reseeds from actual brightness", () =>
    {
        ...
        Require(!result.ManualOverride); Require(display.Level == 30);
    ...
    {
        using var a = new FakeDisplay("a", 50); using var b = new FakeDisplay("b", 50);
        var coordinator = new BrightnessCoordinator([new(a, new()), new(b, new(false))], new());
    ...
    ("Room preferences preserve independent levels without a startup jump", () =>
    {
        ...
        var coordinator = new BrightnessCoordinator([
            new(a, new(true, 10, 100, 50, 30)), new(b, new(true, 10, 100, 70, 30))], new(), true, true);
    ...
    ("A fresh room reference starts in darkness or saturation without changing preferences", () =>
    {
        foreach (var light in new[] { 0d, 0.7916666667, 2.9, 248, 255 })
        {
            ...
            var coordinator = new BrightnessCoordinator([new(a, aProfile), new(b, bProfile)], new(), true, true);
    ...
    ("A dark starting reference still follows a later increase in light", () =>
    {
        ...
        var coordinator = new BrightnessCoordinator([new(a, profile)], new(), true, true);
        ...
        Near(profile.Target(20, 0), a.Level, 1); Require(a.Level > 70);
    ...
    ("Relative room mapping is bounded and reaches full external brightness", () =>
    {
        ...
        Near(70, profile.Target(20, 0)); Near(88, profile.Target(48, 0)); Near(100, profile.Target(255, 0));
        Near(10, new DisplayProfile(true, 10, 100, 20, 100).Target(0, 0));
        for (var n = 1; n <= 255; n++) Require(profile.Target(n, 0) >= profile.Target(n - 1, 0));
    ...
    ("A sustained dark scene dims both screens gently and light restores them", () =>
    {
        using var a = new FakeDisplay("a", 50); using var b = new FakeDisplay("b", 70);
        var coordinator = new BrightnessCoordinator([
            new(a, new(true, 10, 100, 50, 40)), new(b, new(true, 10, 100, 70, 40))], new(), true, true);
    ...
    ("Native manual changes teach one screen without stopping the other", () =>
    {
        using var a = new FakeDisplay("a", 50); using var b = new FakeDisplay("b", 70);
        var coordinator = new BrightnessCoordinator([
            new(a, new(true, 10, 100, 50, 20)), new(b, new(true, 10, 100, 70, 20))], new(), true);
        coordinator.Tick(48, TimeSpan.Zero); a.Level = 35;
        var cycle = coordinator.Tick(48, TimeSpan.FromSeconds(1));
        Require(!cycle.ManualOverride); Near(35, a.Level); Require(b.Level > 70);
    ...
    ("Slider preferences are immediate and are not fought by automatic mode", () =>
    {
        ...
        var coordinator = new BrightnessCoordinator([new(a, new(true, 10, 85, 50, 40))], new(), true, true);
        coordinator.Tick(40, TimeSpan.Zero);
    ...
    ("Delayed monitor readback cannot silently recalibrate room preferences", () =>
    {
        ...
        for (var i = 0; i < 1200; i++)
        {
            ...
            Require(!cycle.ManualOverride && cycle.Displays.All(d => d.LearnedProfile is null));
    ...
    ("Dark and bright camera steps finish within three seconds without a jump", () =>
    {
        foreach (var (start, reference, nextLight) in new[] { (95, 180d, 0d), (10, 3d, 255d), (50, 40d, 0d), (30, 0d, 40d) })
        {
            ...
            var expected = (int)Math.Round(profile.Target(nextLight, 0));
    ...
    ("Reusing a camera frame cannot turn one flash into a lasting light change", () =>
    {
        ...
        var coordinator = new BrightnessCoordinator([new(screen, new(true, 10, 100, 50, 40))], new(), true, true);
    ...
    ("Quantized hardware levels are not mistaken for manual changes", () =>
    {
        ...
        var coordinator = Make(display);
        for (var i = 0; i < 30; i++) Require(!coordinator.Tick(200, TimeSpan.FromSeconds(i)).ManualOverride);
...
static BrightnessCoordinator Make(params FakeDisplay[] displays) => new(displays.Select(d => new DisplayPlan(d, new DisplayProfile(true, 0, 100))), new Calibration(0, 200, 1));
