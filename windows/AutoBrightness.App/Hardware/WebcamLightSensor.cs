...
public sealed class WebcamLightSensor : IAsyncDisposable
{
    ...
    public async Task StartAsync(string cameraId, bool shared, double exposureMilliseconds)
    {
        await StopAsync();
        try
        {
            ...
            if (!shared)
            {
                ...
                if (exposure.Supported)
                {
                    ...
                    ModeDescription = $"Exposure locked at {exposure.Value.TotalMilliseconds:0.##} ms. Calibrate for this camera and position.";
                }
                else
                {
                    ...
                    ModeDescription = $"Exposure locked at {1000 * Math.Pow(2, actual):0.##} ms (UVC). Calibrate for this camera and position.";
                    _lockedLegacyValue = actual;
                }
            }
            else ModeDescription = "Shared camera: exposure may change with other apps. Calibrate and check stability.";
    ...
    private void FrameArrived(MediaFrameReader sender, MediaFrameArrivedEventArgs args)
    {
        ...
        if (now - Interlocked.Read(ref _lastFrame) < 200 || Interlocked.CompareExchange(ref _processing, 1, 0) != 0) return;
        try
        {
            ...
            var data = new byte[checked(image.PixelWidth * image.PixelHeight * 4)];
            var buffer = new Windows.Storage.Streams.Buffer((uint)data.Length);
            image.CopyToBuffer(buffer);
            using var reader = DataReader.FromBuffer(buffer);
            reader.ReadBytes(data);
            var measurement = LightMeter.MeasureSceneBgra(data, image.PixelWidth, image.PixelHeight);
